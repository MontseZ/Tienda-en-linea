...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $comprasmes=DB::select('SELECT month(c.purchase_date) as mes, sum(c.monto) as totalmes from purchases c group by month(c.purchase_date) order by month(c.purchase_date) desc limit 12');
        ...
        $totales=DB::select('SELECT (select ifnull(sum(c.monto),0) from purchases c where DATE(c.purchase_date)=curdate()) as totalcompra, (select ifnull(sum(v.total),0) from sales v where DATE(v.sale_date)=curdate() and v.status="VALID") as totalventa');
